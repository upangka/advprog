...
## 使用 WriteStream 的正确异步批处理

使用流，可以看到时间明显下降

[exercise_05.ts](./code/exercise_05.ts)是简化版，只说明原理，完整优化版本见[node背压机制](./code/exercise_06.ts)
...
| :-------------------------- | :---------- | :------------ | :------------------ | :----------------------------------------------------------------- |
| `await write`（异步串行化） | **~90s**    | ✅ **完整**   | `exercise_02.ts`    | 每次写入阻塞等待系统调用完成，串行执行导致性能极差。               |
| `writeSync`（同步循环）     | **~1.92s**  | ✅ **完整**   | `exercise_03.ts`    | 同步阻塞，直接发起系统调用，无中间层开销。                         |
| `write`（不关闭，无等待）   | **~1.895s** | ❌ **不完整** | `exercise_04.ts`    | 异步非阻塞，仅提交任务不等待完成，数据大概率丢失。                 |
| `WriteStream`（缓冲写入）   | **~339ms**  | ✅ **完整**   | `exercise_05.ts\*\* | 异步缓冲，主线程写入内存缓冲区，后台批量落盘。性能最优且数据完整。 |
